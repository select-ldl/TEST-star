...
def data_iftype(type):
    ...
    return dt
def data_iftext(text,name,Materials,moble,size,color,quantity,packaging,whys,weight,img):
    """商品详情"""
    ...
    if text1=='nan':
        ...
        if color != "":
            ...
            bok = bok + b
        if Materials != "":
            b = f'<p>Materials:{(Materials)}</p>'
            bok = bok + b
        if quantity!="":
            b=f"<p>Box specifications:{(quantity)}/BOX</p>"
            bok=bok+b
        if packaging != "":
            b = f'<p>Packaging method:{(packaging)}</p>'
            bok = bok + b
        if weight != "":
            b = f'<p>Product weight:{(weight)}</p>'
            bok = bok + b
        if whys != "":
...
def data_code():
    ...
    Materials = (pd.read_excel(init_fille()[0], usecols=[13]).values)#材料
    ...
    for k in range(len_te):
        ...
        payt = {
            ...
            "Materials": data_if(Materials[k][0]),#材质
            ...
            "text":data_iftext(img=data_if(text_img[k][0]),weight=data_if(weight[k][0]),whys=data_if(whys[k][0]),color=data_if(color[k][0]),packaging=data_if(Packaging[k][0]),size=data_if(size[k][0]),quantity=data_if(quantity[k][0]),text=text[k][0],moble=data_if(moble[k][0]),name=(title[k][0]),Materials=data_if(Materials[k][0])),#文本
...
def list_children(color,size,Materials,modble):
    data=[]
    modble=list_spilst(modble)
    color = list_spilst(color)
    size = list_spilst(size)
    Materials = list_spilst(Materials)
    sku_price={}
    # print(color)
    # sku_len=len(modble)+len(color)+len(size)+len(Materials)
    sku_id_m = 1
    if modble !="":
        abcd = []
        sku_id=sku_id_m
    ...
    if Materials != "":
        abcd = []
        sku_id=sku_id_m
        for dilid in range(0, len(Materials)):
            sku_id_m = sku_id_m + 1
            afile = {"id": 0, "temp_id": sku_id_m,
                     "name": Materials[dilid], "pid": 0}
            abcd.append(afile)
            sku_i = {str([str(sku_id_m), Materials[dilid]]): "Materials"}
            sku_price.update(sku_i)
        data_Materials = {"id": 0, "temp_id":sku_id, "name": "Materials", "pid": 0,
                      "children": (abcd)}
        data.append(data_Materials)
        # d={"Materials",sku_da}
        # sku_price.update()
    if size != "":
        abcd = []
...
def sku_len(color,size,Materials,model):
    # arrays={}
    abcd=[]
    if model!=[]:
        abcd.append(model)
        ...
        # arrays.update(a)
    if Materials!=[]:
        abcd.append(Materials)
        # a = {"Materials,"+str(len(size)):Materials}
        # arrays.update(a)
    if size!=[]:
        abcd.append(size)
...
def sku_json(t_id,price,up_down,id_text,id_ids,img):
    data_m = {"id": 0,
              "temp_id": t_id,
              "goods_sku_ids": "",
              "goods_id": 0,
...
def listgoods_sku(color,size,Materials,model,price,price_sku):
    data_m=list_children(color=color,size=size,Materials=Materials,modble=model)[0]
    aaaa=list_children(color=color,size=size,Materials=Materials,modble=model)[1]
    # print(len(aaaa))
    sku_color=[]
    sku_size=[]
    sku_Materials=[]
    ...
    for anga in aaaa:
        ...
        if aaaa[anga]=="Materials":
            sku_Materials.append(anga)
        if aaaa[anga] == "size":
    ...
    skulen = sku_len(color=sku_color, size=sku_size, Materials=sku_Materials, model=sku_model)
    # print(len(skulen),skulen)
    price_sku=sku_price(price_sku)
    if price_sku == "":
        for slu in range(len(skulen)):
            sk1=sku_json(t_id=slu+1,up_down="up",price=price,id_text=skulen[slu][1],id_ids=skulen[slu][0],img=None)
            sku_app.append(sk1)
        # print(slu,)
    else:
        idsd=1
        ...
        for ais in price_sku:
            ...
            for poi in skulen:
                if ais[:-2]==poi[1]:
                    sk1 = sku_json(t_id=idsd, up_down="up", price=round(float(ais[-2]),2), id_text=poi[1],
                                   id_ids=poi[0],img=ais[-1])
                    sku_app.append(sk1)
                    idsd=idsd+1
        ...
        for iop in new_amm:
            for ak in skulen:
                if ak[1]==iop:
                    sk1 = sku_json(t_id=idsd, up_down="down", price=price, id_text=ak[1], id_ids=ak[0],img=None)
                    sku_app.append(sk1)
                    idsd=idsd+1
        print(sku_app)
        # print(new_amm)
...
def up_data_M(Materials,moble,sku_price,text,type,img,size,color,imgs,price,price1,price2,inventory,pop,volume,supplier,sort,subtitle,title,titleen,titletw,weight,):
    data = {
        "row[category_ids]":type,#商品类型69,179,183,187,186,194,198,248,316
        "row[content]":text,#"<p>图文详情<br/></p>"
        "row[dispatch_ids]":"1",
        ...
        "row[autosend_content]":"",
        "row[zenggoods_id]":"",
        "sku[listData]":(listgoods_sku(size=size, color=color,Materials=Materials,model=moble,price=price,price_sku=sku_price))[0],
        "sku[priceData]":(listgoods_sku(size=size, color=color,Materials=Materials,model=moble,price=price,price_sku=sku_price))[1]}
    data["row[cost_price]"]=""
    return data
def test_index():
    datas=data_code()
    ...
    for i in range(len(datas)):
        if datas[i]["sku_l_m"]=="0":
            ...
            back = up_data_M(supplier=datas[i]["supplier"], title=datas[i]["title"], titleen="", titletw="",
                         ...
                         weight=datas[i]["weight"], subtitle=datas[i]["subtitle"], price1=datas[i]["price1"],
                         imgs=datas[i]["imgs"], img=datas[i]["img"], price=datas[i]["price"], price2=datas[i]["price2"],pop="",
                         sort=datas[i]["sort"],sku_price=datas[i]["sku_price"],Materials=datas[i]["Materials"],
                         text=datas[i]["text"], inventory=datas[i]["inventory"], volume="",)
            pat_all.append(back)
    return pat_all
